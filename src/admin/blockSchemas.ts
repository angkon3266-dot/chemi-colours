...
export const BLOCK_SCHEMAS: Record<BlockType, BlockSchema> = {
  hero: {
    ...
    fields: [
      ...
      { key: 'headlineColor', label: 'Line 1 colour', type: 'color', hint: 'Leave as white unless you want this line to stand out.' },
      { key: 'headline2', label: 'Headline — line 2', type: 'text' },
      { key: 'headline2Color', label: 'Line 2 colour', type: 'color' },
      { key: 'accent', label: 'Headline — line 3', type: 'text', hint: 'Optional. Each line always renders on its own row, on every screen size.' },
      { key: 'accentColor', label: 'Line 3 colour', type: 'color' },
    ...
    defaults: {
      ...
      headline2: '', headline2Color: '#ffffff',
      accent: '', accentColor: '#ffffff',
      useSiteVideo: true, videoUrl: '',
      ...
      },
      {
        ...
        itemFields: [
          { key: 'image', label: 'Image', type: 'media', mediaKind: 'image' },
          {
            ...
            hint: 'Shown in the corner over the photo. Leave blank for none.',
            options: [
              { value: '', label: 'None' },
              { value: 'shirt', label: 'Shirt (apparel)' },
              { value: 'layers', label: 'Layers (knits, laminates)' },
              { value: 'weave', label: 'Weave (woven fabric)' },
              { value: 'sofa', label: 'Sofa (home & upholstery)' },
              { value: 'waves', label: 'Waves (absorbency, water)' },
              { value: 'zap', label: 'Zap (performance)' },
              { value: 'droplet', label: 'Droplet (dyeing, wet processing)' },
              { value: 'droplets', label: 'Droplets (finishing)' },
              { value: 'package', label: 'Package (bulk, packaging)' },
              { value: 'sparkles', label: 'Sparkles (brightening, finish)' },
              { value: 'palette', label: 'Palette (colour)' },
              { value: 'home', label: 'Home (interiors)' },
              { value: 'scissors', label: 'Scissors (cut & sew)' },
              { value: 'umbrella', label: 'Umbrella (outdoor, technical)' },
            ],
          },
          { key: 'title', label: 'Title', type: 'text' },
          { key: 'text', label: 'Text', type: 'textarea' },
